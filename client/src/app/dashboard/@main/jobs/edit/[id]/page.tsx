...
export default function EditJob({ params }: { params: { id: string } }) {
  ...
  const { data: job, isLoading,isSuccess } = useQuery({
  ...
  useEffect(()=>{
    ...
    form.setValue("job_description",job?.job_description)
    form.setValue("job_mode",job?.job_mode)
    form.setValue("job_type",job?.job_type)
    form.setValue("job_category",job?.job_category)
    form.setValue("job_industry",job?.job_industry)
    form.setValue("job_requirements",job?.job_requirements)
    form.setValue("experience",job?.experience)
    form.setValue("salary",job?.salary)
    form.setValue("skill_rquired",job?.skill_rquired)
    form.setValue("duration",job?.duration)
    form.setValue("applications_start_date",job?.applications_start_date)
    form.setValue("applications_end_date",job?.applications_end_date)
    setSkills(job?.skill_rquired)
    setRequirements(job?.job_requirements)
  },[isSuccess])
